...
import navigateToListView from '@salesforce/apex/gapProcessSelectedCarsController.getListViewDetails';
...
export default class GapProcessSelectedCars extends LightningElement {
    @track records = [];
    @track error;
    @api recordId;
    ...
    @track columns = [
        ...
        iconPosition: 'left' } },
        { type: 'button-icon',label: 'Update Lines',
        initialWidth: 150,
        title:'Update Lines',
        typeAttributes: {iconName:'utility:add', label: 'Update Lines', 
        name: 'Mass_Update', variant: 'brand',
        iconPosition: 'left' } }
        /*,
        ...
        switch (actionName) {
            case 'Select_As_Is':
             ...
             case 'Need_Configuration':   
                ...
                break;
            case 'Mass_Update':
                this.navigateToMassUpdate(row);
                break;

          }
     }
     navigateToMassUpdate(tableRow){
        var groupId = tableRow.Id;
        this.isLoaded = false;
        console.log('groupId>>>>>>>',groupId);
        navigateToListView({assetGroupId: groupId}).then(result => {
            if(result.success){
                    var listViewId = result.listViewId;
                    console.log('listViewId>>>>>>>',listViewId);
                    window.open('/lightning/o/Asset_Lines__c/list?filterName='+listViewId,'_blank');
                this.isLoaded = true;
            }else{
    ...
    showAssetLines(tableRow){
        var groupId = tableRow.Id;
        console.log('groupId>>>>>>>>>>',groupId);
        console.log('groupId>>>>>>>>>>',this.vfPageURL);
        this.finalVfURL = this.vfPageURL+'/apex/AssetGroup?Id='+groupId;
        //alert(this.finalVfURL);
